...
async function renderClusters() {
    ...
    if (pieCtx) {
        ...
        // If all zeros, show no-data message instead of fake equal split
        const hasData = dist.Low > 0 || dist.Medium > 0 || dist.High > 0;

        if (!hasData) {
            if (pieCtx) {
                pieCtx.parentElement.innerHTML = '<p style="color:#666;text-align:center;padding:2rem;font-size:0.85rem;">Upload energy data to see usage clusters.</p>';
            }
        } else {
            const pieData  = [dist.Low, dist.Medium, dist.High];
            const pieColors = ['#b8960c', '#FFD700', '#ff6b35'];
...
// ── Room-wise Consumption (from submetering API) ──────────────────────
async function renderRoomWise() {
    const panel = document.getElementById('roomWisePanel');
    if (!panel) return;
    try {
        ...
        if (!data.hasData) {
            panel.innerHTML = '<p style="color:#666;text-align:center;padding:1rem;font-size:0.85rem;">Upload energy data to see room-wise breakdown.</p>';
            return;
        }

        const categories = [
            { label: 'HVAC',     icon: 'snowflake', kwh: data.hvac?.kwh     || 0, color: '#FFD700' },
            { label: 'Kitchen',  icon: 'utensils',  kwh: data.kitchen?.kwh  || 0, color: '#ff6b35' },
            { label: 'Laundry',  icon: 'tshirt',    kwh: data.laundry?.kwh  || 0, color: '#4facfe' },
            { label: 'Lighting', icon: 'lightbulb', kwh: data.lighting?.kwh || 0, color: '#43e97b' },
            { label: 'Other',    icon: 'home',      kwh: data.other?.kwh    || 0, color: '#b8960c' }
        ].filter(c => c.kwh > 0).sort((a, b) => b.kwh - a.kwh);

        const maxKwh = categories[0]?.kwh || 1;
        const note   = data.estimated ? '<p style="color:#555;font-size:0.75rem;margin-top:0.5rem;">* Estimated from usage time patterns</p>' : '';

        panel.innerHTML = `<div class="room-analysis">${
            categories.map(c => {
                const pct = Math.round((c.kwh / maxKwh) * 100);
                return `<div class="room-item">
                    ...
                    <div class="room-bar">
                        <div class="room-progress" style="width:${pct}%;background:${c.color};"></div>
                    </div>
                    <span class="room-value">${c.kwh.toFixed(1)} kWh</span>
                </div>`;
            }).join('')
        }</div>${note}`;
    } catch (e) {
        panel.innerHTML = '<p style="color:#666;text-align:center;padding:1rem;font-size:0.85rem;">Room data unavailable.</p>';
    }
}
