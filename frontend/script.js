...
async function loadUserDashboardStats(summary) {
    ...
    // Update stat change labels from real data
    ...
    // Load AI insights from recommendations API
    loadAIInsights();

    // Load real active devices
...
async function initializeRealtimeChart() {
    ...
    // Load real hourly averages from backend
    let hourlyData = [];
    const token = getToken();
    if (token) {
        try {
            ...
            if (data.hasData && data.hourly?.length) {
                hourlyData = data.hourly; // [{hour, avg_kw, total_kwh}]
            }
        } catch (e) { /* use empty */ }
    }

    // Build 24-point chart from real hourly averages (0–23)
    const now = new Date();
    const currentHour = now.getHours();

    // Show last 8 hours up to current hour
    const hoursToShow = [];
    for (let i = 7; i >= 0; i--) {
        hoursToShow.push((currentHour - i + 24) % 24);
    }

    const labels = hoursToShow.map(h => h.toString().padStart(2, '0') + ':00');
    const chartData = hoursToShow.map(h => {
        const entry = hourlyData.find(d => d.hour === h);
        return entry && entry.avg_kw > 0 ? entry.avg_kw : null;
    });

    // Determine y-axis scale from real data
    const validVals = chartData.filter(v => v !== null);
    const baseValue = validVals.length ? validVals.reduce((a, b) => a + b, 0) / validVals.length : 1.0;
    const yMin = 0;
    const yMax = validVals.length ? parseFloat((Math.max(...validVals) * 1.3).toFixed(2)) : 3;
    ...
    window.realtimeChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: c => c.raw !== null ? ` ${c.raw} kW (hourly avg)` : ' No data'
                    }
                }
            },
            scales: {
                ...
                x: { grid: { display: false }, ticks: { color: '#aaaaaa', maxTicksLimit: 8 } }
            }
        }
    });

    // Store base for live update
    window._realtimeBase = baseValue;
}
...
function startLiveUpdate() {
    // Refresh real data every 5 minutes instead of random simulation
...
async function refreshRealtimeChart() {
    ...
    try {
        ...
        const currentHour = now.getHours();
        const hoursToShow = [];
        for (let i = 7; i >= 0; i--) hoursToShow.push((currentHour - i + 24) % 24);

        chart.data.labels = hoursToShow.map(h => h.toString().padStart(2, '0') + ':00');
        chart.data.datasets[0].data = hoursToShow.map(h => {
            const entry = data.hourly.find(d => d.hour === h);
            return entry && entry.avg_kw > 0 ? entry.avg_kw : null;
        });
        chart.update('none');
        updateCurrentUsage();
    } catch (e) { /* keep existing chart */ }
}
...
function updateCurrentUsage() {
    ...
    if (usageElement && window.realtimeChart) {
        ...
        if (validVals.length) {
            usageElement.textContent = validVals[validVals.length - 1] + ' kW';
        }
    }
}
