...
async function initializeSavingsChart() {
    const ctx = document.getElementById('savingsChart');
    if (!ctx) return;
    ctx.parentElement.style.height   = '140px';
    ctx.parentElement.style.position = 'relative';

    // Build savings projection from actual recommendations data
    ...
    try {
        ...
        const data = await res.json();
        if (data.hasData && data.recommendations?.length) {
            // Sum up potential weekly savings from all recommendations
            const totalWeeklySavings = data.recommendations
                .filter(r => r.potential_savings_inr != null)
                .reduce((s, r) => s + (parseFloat(r.potential_savings_inr) || 0), 0);

            if (totalWeeklySavings > 0) {
                // Progressive weekly savings (20%, 45%, 75%, 100% adoption)
                const adoptionRate = [0.20, 0.45, 0.75, 1.0];
                savingsData = adoptionRate.map(r => parseFloat((totalWeeklySavings * r).toFixed(2)));
                targetData  = adoptionRate.map(() => parseFloat(totalWeeklySavings.toFixed(2)));
            }
        }
    } catch (e) { /* empty */ }

    if (!savingsData.length) {
        ctx.parentElement.innerHTML = '<p style="color:#666;text-align:center;padding:2rem;font-size:0.85rem;">Upload energy data to track savings progress.</p>';
        return;
    }
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: { display: true, labels: { color: '#aaaaaa', usePointStyle: true, padding: 10, font: { size: 10 } } }
            },
            scales: {
