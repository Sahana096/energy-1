...
const getRangeData = async (req, res, next) => {
  try {
    ...
    // Check if data is mostly monthly (bill data) - look for dates on same day of month (e.g., all 15th)
    const isMonthlyData = docs.length > 0 && 
      docs.every(d => d.date.getDate() >= 10 && d.date.getDate() <= 20) &&
      docs.length >= 1;
    ...
    // If we have only monthly bill data, show 'no data' for day/week/year views
    if (isMonthlyData && ['day', 'week', 'year'].includes(period)) {
      return res.json({ success: true, hasData: false, labels: [], prevLabels: [], current: [], previous: [], unit: 'kWh', period });
    }
    ...
    if (period === 'day') {
      ...
      unit = 'kWh';
      // For monthly view, group by month/year instead of individual days
      const monthMap = {};
      docs.forEach(d => {
        const key = d.date.getFullYear() + '-' + String(d.date.getMonth() + 1).padStart(2, '0');
        monthMap[key] = (monthMap[key] || 0) + d.energyConsumed;
